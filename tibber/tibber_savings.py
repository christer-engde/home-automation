...
# print("len(tibber_bearers) : {}".format(len(tibber_bearers)))
# print("len(slack_hooks)    : {}".format(len(slack_hooks)))
...
for bearer_index in range(0, len(tibber_bearers)):
    ...
    for home in tibber_response["data"]["viewer"]["homes"]:
        ...
        for duration in range(4, 2, -1):
            print("### Handle duration {}".format(duration))
            ...
            for hour_index in range(index_start, len(arr_timestamps)):
                print("{}: {}".format(str(hour_index), arr_timestamps[hour_index]), end='')
                hour_iter = hour_index
                print(" {} ({:5.2f})".format(str(hour_iter), arr_prices[hour_iter]), end='')
                ...
                print(" " + str("{:5.2f}".format(average)), end='')
                ...
                if (average < average_lowest):
                    ...
                    if (arr_prices[hour_index] <= arr_prices[hour_index + duration - 1]):
                        average_timestamp_info = "\n\n{}h starta kl. {} ({:.2f} kr/kWh)\n".format(str(duration), str(datetime.datetime.strptime(average_timestamp_start, "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%H:%M")), average_lowest)
                    else:
                        average_timestamp_info = "\n\n{}h avsluta kl. {} ({:.2f} kr/kWh)\n".format(str(duration), str(datetime.datetime.strptime(average_timestamp_end,   "%Y-%m-%dT%H:%M:%S.%f%z").strftime("%H:%M")), average_lowest)
                else:
                    print("       ", end='')

                if (hour_index == index_start):
                    average_timestamp_compare = "Jmf kl. {}-{} ({:.2f} kr/kWh)\n".format(
            ...
            print("A")
            print(average_timestamp_info)
            message_to_post += average_timestamp_info + average_timestamp_compare + average_timestamp_info_highest

        #message_to_post += average_timestamp_info_highest
...
################################################################################################################################################################
    ...
    if slack_hooks[bearer_index] != None:
        ...
        if (dummy_data == True):
            slack_message_to_post = ("Dummy data\n\n" + message_to_post).replace("\n", "\\\\n")
        else:
